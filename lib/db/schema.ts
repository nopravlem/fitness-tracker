...
export const dailyCheckins = pgTable("daily_checkins", { id: serial("id").primaryKey(), checkinDate: date("checkin_date").notNull().unique(), proteinTargetMet: boolean("protein_target_met"), calorieRangeMet: boolean("calorie_range_met"), hydrationTargetMet: boolean("hydration_target_met"), energy: integer("energy"), sleepHours: numeric("sleep_hours", { precision: 4, scale: 1 }), notes: text("notes") });
