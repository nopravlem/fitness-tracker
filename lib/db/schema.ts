import { integer, numeric, pgTable, serial, text, timestamp, date, boolean } from "drizzle-orm/pg-core";
...
export const exerciseSets = pgTable("exercise_sets", {
  ...
  assistance: numeric("assistance", { precision: 7, scale: 2 }),
...
export const cardioSessions = pgTable("cardio_sessions", {
  ...
  nextDayPain: integer("next_day_pain"),
  notes: text("notes"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});
...
export const mobilitySessions = pgTable("mobility_sessions", {
  id: serial("id").primaryKey(),
  performedOn: date("performed_on").notNull(),
  hip: boolean("hip").notNull().default(false),
  ankleCalf: boolean("ankle_calf").notNull().default(false),
  neckUpperBack: boolean("neck_upper_back").notNull().default(false),
  splitPractice: boolean("split_practice").notNull().default(false),
  minutes: integer("minutes"),
  stiffness: integer("stiffness"),
  notes: text("notes"),
});
...
export const progressMetrics = pgTable("progress_metrics", {
  ...
  weightLb: numeric("weight_lb", { precision: 5, scale: 1 }),
  waistIn: numeric("waist_in", { precision: 5, scale: 2 }),
  pushups: integer("pushups"),
  pullupAssistanceLb: numeric("pullup_assistance_lb", { precision: 6, scale: 1 }),
  pullups: integer("pullups"),
  mileSeconds: integer("mile_seconds"),
  splitDistanceIn: numeric("split_distance_in", { precision: 5, scale: 2 }),
  notes: text("notes"),
});
