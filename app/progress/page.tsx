...
type WorkoutRow={id:number;date:string;sessionType:string;completed:boolean;energy:number|null;limited:boolean;limitationNotes:string|null};
...
export default function ProgressPage(){
  const[rows,setRows]=useState<ProgressRow[]>([]);const[workouts,setWorkouts]=useState<WorkoutRow[]>([]);const[error,setError]=useState<string|null>(null);const[loading,setLoading]=useState(true);const[editingId,setEditingId]=useState<number|null>(null);const[form,setForm]=useState<ProgressForm>(emptyForm());const[saving,setSaving]=useState(false);
  async function load(){setLoading(true);try{const[trackerR,workoutR]=await Promise.all([fetch('/api/tracker'),fetch('/api/workouts/today?history=1')]);if(!trackerR.ok){const j=await trackerR.json().catch(()=>null);throw new Error(j?.error||`Load failed (${trackerR.status})`)}if(!workoutR.ok){const j=await workoutR.json().catch(()=>null);throw new Error(j?.error||`Workout history failed (${workoutR.status})`)}const[data,workoutData]=await Promise.all([trackerR.json(),workoutR.json()]);setRows(data.progress||[]);setWorkouts(workoutData.sessions||[]);setError(null)}catch(e){setError(e instanceof Error?e.message:'Unable to load progress')}finally{setLoading(false)}}
  useEffect(()=>{load()},[]);
  const latest=rows[0];
  ...
  async function removeWorkout(row:WorkoutRow){if(!window.confirm(`Delete workout from ${row.date}?`))return;setError(null);const r=await fetch(`/api/workouts/today?date=${row.date}`,{method:'DELETE'});if(!r.ok){const j=await r.json().catch(()=>null);setError(j?.error||`Delete failed (${r.status})`);return}await load()}

  return <main className="shell">
    <header><div><p className="eyebrow">SADHANA</p><h1>Progress</h1><p className="subtle">Long-view progress plus the history you may need to correct.</p></div></header>
    {error&&<section className="card errorCard"><p>{error}</p></section>}
    <section className="card"><p className="eyebrow">CURRENT SNAPSHOT</p><div className="metrics"><div><strong>Push-ups</strong><span>{latest?.pushups ?? '—'} floor reps</span></div><div><strong>Pull-up</strong><span>{latest?.pullups ?? 0} unassisted · {latest?.pullupAssistanceLb??'—'} lb assist</span></div><div><strong>Mile</strong><span>{mileLabel(latest?.mileSeconds??null)}</span></div><div><strong>Split distance from floor</strong><span>{latest?.splitDistanceIn??'—'} in</span></div></div></section>

    {editingId!==null&&<section className="card progressEditorCard"><div className="progressEditor"><p className="eyebrow">{editingId===0?'NEW CHECK-IN':'EDIT CHECK-IN'}</p><h2>{editingId===0?'Add progress':'Update progress'}</h2><div className="formGrid" style={{gridTemplateColumns:'repeat(auto-fit,minmax(min(220px,100%),1fr))'}}><label>Date<input type="date" value={form.measuredOn} style={{display:'block',width:'100%',maxWidth:'100%',minWidth:0,WebkitAppearance:'none',appearance:'none'}} onChange={e=>update('measuredOn',e.target.value)}/></label><label>Weight (optional)<input inputMode="decimal" value={form.weightLb} onChange={e=>update('weightLb',e.target.value)}/></label><label>Waist (optional)<input inputMode="decimal" value={form.waistIn} onChange={e=>update('waistIn',e.target.value)}/></label><label>Floor push-ups<input inputMode="numeric" value={form.pushups} onChange={e=>update('pushups',e.target.value)}/></label><label>Pull-up assistance lb<input inputMode="decimal" value={form.pullupAssistanceLb} onChange={e=>update('pullupAssistanceLb',e.target.value)}/></label><label>Unassisted pull-ups<input inputMode="numeric" value={form.pullups} onChange={e=>update('pullups',e.target.value)}/></label><label>Mile seconds<input inputMode="numeric" value={form.mileSeconds} onChange={e=>update('mileSeconds',e.target.value)}/></label><label>Split distance from floor (in)<input inputMode="decimal" value={form.splitDistanceIn} onChange={e=>update('splitDistanceIn',e.target.value)}/></label></div><textarea placeholder="Optional note" value={form.notes} onChange={e=>update('notes',e.target.value)}/><div className="editorActions"><button className="secondary" disabled={saving} onClick={save}>{saving?'Saving…':'Save check-in'}</button><button className="ghostButton" onClick={cancelEdit}>Cancel</button></div></div></section>}

    <section className="card progressManage"><div className="progressManageHead"><div><p className="eyebrow">WORKOUT HISTORY</p><h2>Workouts</h2></div><span className="subtle">{workouts.length} saved</span></div>
      {loading?<p className="subtle">Loading…</p>:workouts.length===0?<p className="subtle">No workouts saved yet.</p>:workouts.map(row=><div className="historyRow" key={row.id}><div className="historyRowHead"><div><b>{row.date}</b><span>{row.sessionType} · {row.completed?'completed':'partial'}{row.limited?' · adapted':''}</span></div><div className="historyActions"><Link className="ghostButton" href={`/progress/workouts/${row.date}`}>Edit</Link><button className="dangerText" onClick={()=>removeWorkout(row)}>Delete</button></div></div>{row.limitationNotes&&<p>{row.limitationNotes}</p>}</div>)}
    </section>

    <section className="card progressManage"><div className="progressManageHead"><div><p className="eyebrow">PROGRESS HISTORY</p><h2>Check-ins</h2></div><button className="compactButton" onClick={startAdd}>+ Add</button></div>
