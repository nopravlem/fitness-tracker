...
export default function Home(){
  ...
  return <main className="shell">
    ...
    {todayPlan.exercises&&<><section className="card energy"><h3>How did the workout feel?</h3><div>{[1,2,3,4,5].map(n=><button key={n} className={log.energy===n?'active':''} onClick={()=>{setSaved(false);setLog({...log,energy:n})}}>{n}</button>)}</div><p>1 = wrecked · 5 = could do more</p></section><button className="finish" disabled={saving} onClick={()=>{const n={...log,done:true,sessionType:todayPlan.key};setLog(n);saveWorkout(n)}}>{saving?'Saving…':saved?'✓ Workout saved':'Finish & save workout'}</button></>}
