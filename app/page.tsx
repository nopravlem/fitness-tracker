...
type Exercise={name:string;target:string;note?:string;sets:SetLog[];plannedSets:number};
...
function buildExercises():Exercise[]{const p=planForDate();return (p.exercises||[]).map(ex=>({name:ex.name,target:ex.target,note:ex.note,plannedSets:ex.sets,sets:Array.from({length:ex.sets},()=>({weight:ex.defaultWeight||'',reps:''}))}))}
...
export default function Home(){
  ...
  const[log,setLog]=useState<DayLog>(freshLog());const[previous,setPrevious]=useState<PreviousRow[]>([]);const[saving,setSaving]=useState(false);const[autoSaveState,setAutoSaveState]=useState<'idle'|'saving'|'saved'>('idle');const[loaded,setLoaded]=useState(false);const[dbError,setDbError]=useState<string|null>(null);const[saved,setSaved]=useState(false);const[notice,setNotice]=useState<string|null>(null);const[progressRows,setProgressRows]=useState<ProgressRow[]>([]);const[menuOpen,setMenuOpen]=useState(false);const lastPersisted=useRef('');
  ...
  ]).then(([workoutData,trackerData])=>{setPrevious(workoutData.previous||[]);setProgressRows(trackerData.progress||[]);if(workoutData.log){const next=freshLog();next.done=workoutData.log.done;next.energy=workoutData.log.energy;for(const row of workoutData.log.rows){const ex=next.exercises.find(e=>e.name===row.exerciseName);if(ex){while(ex.sets.length<row.setNumber)ex.sets.push({weight:'',reps:''});ex.sets[row.setNumber-1]={weight:row.weight||'',reps:row.reps?.toString()||''}}}lastPersisted.current=JSON.stringify(next);setLog(next);setSaved(next.done)}else{lastPersisted.current=JSON.stringify(freshLog())}setDbError(null);setLoaded(true)}).catch(e=>{setDbError(e instanceof Error?e.message:'Unable to sync with database');setLoaded(true)});},[]);

  useEffect(()=>{if(!loaded||!todayPlan.exercises)return;const serialized=JSON.stringify(log);if(serialized===lastPersisted.current)return;setAutoSaveState('idle');const timer=window.setTimeout(async()=>{setAutoSaveState('saving');try{const r=await fetch('/api/workouts/today',{method:'PUT',headers:{'Content-Type':'application/json'},body:serialized});if(!r.ok){const j=await r.json().catch(()=>null);throw new Error(j?.error||`Autosave failed (${r.status})`)}lastPersisted.current=serialized;setAutoSaveState('saved');setDbError(null)}catch(e){setAutoSaveState('idle');setDbError(e instanceof Error?e.message:'Unable to autosave workout')}},700);return()=>window.clearTimeout(timer)},[log,loaded,todayPlan.exercises]);
  ...
  function addSet(ei:number){setLog(p=>{const n=structuredClone(p);n.exercises[ei].sets.push({weight:'',reps:''});return n})}
  function removeExtraSet(ei:number,si:number){setLog(p=>{const n=structuredClone(p);const ex=n.exercises[ei];if(si>=ex.plannedSets)ex.sets.splice(si,1);return n})}
  ...
  return <main className="shell">
    ...
    <section className="card foodPlan"><div><b>Breakfast</b><p>{food.breakfast}</p></div><div><b>Lunch</b><p>{food.lunch}</p></div><div><b>Dinner</b><p>{food.dinner}</p></div><div className="foodEmergency"><b>If you’re too tired to decide</b><p>{food.emergency}</p></div><p className="note">{food.note}</p></section>

    {todayPlan.exercises&&<><div className="sectionTitle"><h2>Workout</h2><span>{autoSaveState==='saving'?'Saving…':autoSaveState==='saved'?'Saved':loaded?todayPlan.duration:'Loading…'}</span></div>{log.exercises.map((ex,ei)=><section className="card exercise" key={ex.name}><div className="exerciseHead"><div><h3>{ex.name}</h3><p>{ex.target}</p></div><span>{ei+1}</span></div><p className="note"><b>Next target:</b> {suggestion(ex,previous)}</p>{ex.note&&<p className="subtle">{ex.note}</p>}<div className="setHeader"><span>SET</span><span>WEIGHT / ASSIST</span><span>REPS</span></div>{ex.sets.map((s,si)=><div className="setRow" key={si}><b>{si+1}</b><input value={s.weight} placeholder="—" onChange={e=>editSet(ei,si,'weight',e.target.value)}/><div className="repCell"><input inputMode="numeric" value={s.reps} placeholder="—" onChange={e=>editSet(ei,si,'reps',e.target.value)}/>{si>=ex.plannedSets&&<button className="removeSet" type="button" aria-label={`Remove extra set ${si+1}`} onClick={()=>removeExtraSet(ei,si)}>×</button>}</div></div>)}<button className="addSet" type="button" onClick={()=>addSet(ei)}>+ Add set</button></section>)}</>}
